...
exports.searchPokemons = async (req, res) => {
    console.log("Received query parameters:", req.query); // Ajoutez ce log
    console.log("Received query parameters:", req.params); // Ajoutez ce log
    try {
        const searchParams = {
            page: parseInt(req.query.page) || 1,
            size: parseInt(req.query.size) || 20,
            typeOne: req.query.typeOne,
            typeTwo: req.query.typeTwo,
            partialName: req.query.partialName
        };

        console.log("Search parameters:", searchParams); // Ajoutez ce log

        const result = await pokemonService.searchPokemons(searchParams);
